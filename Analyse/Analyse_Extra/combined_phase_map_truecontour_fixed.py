#!/usr/bin/env python3
"""
combined_phase_map_truecontour.py
---------------------------------
...
- TRUE contour lines taken from the underlying numerical fields
  (not column-by-column approximations)
...
def extract_true_contours(x_vals: np.ndarray, y_vals: np.ndarray, Z: np.ndarray, level: float):
    ...
    try:
        ...
        # Most robust access path across matplotlib versions:
        # QuadContourSet.allsegs[level_index] is a list of Nx2 arrays.
        ...
        # Fallback for versions exposing collections/get_paths.
...
def main():
    ...
    if SHOW_BOUNDARY_LINES:
        ...
        for seg in extract_true_contours(x_vals, j_vals, R_grid, rigid_thresh):
            disp = project_true_to_display(seg, x_vals, j_vals)
    ...
    ax.set_xticklabels([f"{1.1547 * v:.2f}" for v in x_vals])
    ...
    handles = [
        mpl.lines.Line2D([0], [0], color=FLOCK_LINE_COLOR, lw=2.7, label=fr"$\langle O\rangle_t={O_THRESHOLD:g}$"),
        mpl.lines.Line2D([0], [0], color=RIGID_LINE_COLOR, lw=2.9, ls="--", label=rigid_legend),
    ...
    outname = f"combined_phase_map_truecontour__{cc_tag}__mode{MODE.lower()}__rigid{RIGIDITY_MODE.lower()}.png"
