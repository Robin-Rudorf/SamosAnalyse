#!/usr/bin/env python3
"""
combined_phase_map_truecontour_underlegend.py
---------------------------------------------
...
- optional suppression of the in-map "Liquid flock" label, with a more
  cautious legend entry instead
...
# If True:
# - the "Liquid flock" text is removed from the map
# - the legend gets an additional cautious label for that red/pink region
MARK_LIQUID_FLOCK_AS_PUTATIVE = False
...
def main():
    ...
    for iy in range(ny):
        for ix in range(nx):
            ...
            if not np.isfinite(o) or not np.isfinite(r):
                continue
            phase_grid[iy, ix] = classify_phase(o >= O_THRESHOLD, r >= high_rigid)
    ...
    if SHOW_BOUNDARY_LINES:
        for seg in extract_true_contours(x_vals, j_vals, O_grid, O_THRESHOLD):
    ...
    if SHOW_REGION_LABELS:
        ...
        for code, (x0, y0) in pos.items():
            if code == 2 and MARK_LIQUID_FLOCK_AS_PUTATIVE:
                continue
            ax.text(
    ...
    handles = [
        mpl.lines.Line2D([0], [0], color=FLOCK_LINE_COLOR, lw=2.7,
                         label=fr"$\langle O\rangle_t={O_THRESHOLD:g}$"),
    ...
    if MARK_LIQUID_FLOCK_AS_PUTATIVE:
        handles.insert(
            2,
            mpl.patches.Patch(
                facecolor=PHASE_COLORS[2],
                edgecolor="none",
                label="putative liquid-flock regime"
            )
        )
    ...
    putative_tag = "putativeLF" if MARK_LIQUID_FLOCK_AS_PUTATIVE else "std"
    outname = f"combined_phase_map_truecontour_underlegend__{cc_tag}__mode{MODE.lower()}__rigid{RIGIDITY_MODE.lower()}__{putative_tag}.png"
    ...
    print("[INFO] Putative liquid-flock legend mode:", MARK_LIQUID_FLOCK_AS_PUTATIVE)
