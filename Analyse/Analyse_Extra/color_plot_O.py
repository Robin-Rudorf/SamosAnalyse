...
CSV_FILE = "../analyse/avg_compute_order.csv"
...
# W-Label
WLABEL=chr(0x27e8)+"O(t)"+chr(0x27e9)

# Title-Prefix
TITLE="flocking-transition diagram, v0=0.05"
...
def plot(csv_file,xcol,ycol,wcol,xlabel,ylabel,wlabel,xsize,ysize,title,scatter,cmap,log,valid_cols):
    ...
    plt.ylabel(ylabel,fontsize=14,rotation=0.1)
    ...
    if log == 1:
        ...
        cbar.ax.set_yticklabels(cy2,fontsize=8)
        cbar.ax.set_ylabel(wlabel,fontsize=14)


    # der 2. Parameter erscheint im Diagramm
    if xlabel=="p0":
        for i in range(len(xval)):
            xval[i]=round(2*float(xval[i])/np.sqrt(3.0),1)
...
def main():
    ...
    for f in files:
        ...
        out = plot(f,xcol,ycol,wcol,xlabel,ylabel,wlabel,xsize,ysize,title,scatter,cmap,log,valid_cols)
        if out:
