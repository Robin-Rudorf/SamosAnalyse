...
def plot_one(df, output, title, xcol, ycol, group_col, style, xsize, ysize, dpi, cmap):
    ...
    for label, sub in groups:
        ...
        if style == "logx":
            plt.semilogx(x, y, marker="o", linewidth=1.5, label=f"{group_col}={label}" if group_col is not None else None)
        elif style == "logxy":
            plt.loglog(x, y, marker="o", linewidth=1.5, label=f"{group_col}={label}" if group_col is not None else None)
        else:
            plt.plot(x, y,  marker="o", linewidth=1.5, label=f"{group_col}={label}" if group_col is not None else None)
...
def main():
    ...
    ap.add_argument("--xcol", default=None, help="Optional: XCol")
    ...
    ap.add_argument("--style", default=None, help="Optional: Style for MSD (line , logx , logxy)")
    ...
    style=args.style or CONFIG.get("style","line")
    xcol=args.xcol or CONFIG.get("xcol")
    ...
    for f in files:
        ...
        out = plot_one(df, output+".png", title, xcol, "Alpha-Relaxaion-Time", group, style, xsize, ysize, dpi, cmap)
        if out:
            print(f"Saved: {out}")
        else:
