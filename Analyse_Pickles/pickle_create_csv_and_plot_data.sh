...
Error=0
python3 pickle2csv.py --csv "${out_dir}/${csv_file}" --split "${split_pl}" ||Error=1
python3 plot_pickle_data.py --csv "${out_dir}/${csv_file}" --group "${group}" --xsize "${x_size}" --title "${title}" --style_msd "${style_MSD}" --style_selfint ${style_SelfInt} --cmap ${colormap} ||Error=1
